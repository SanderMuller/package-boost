...
```php
it('handles edge case with empty skill directory', function () {
    // Arrange: Set up the scenario that triggers the bug
    File::ensureDirectoryExists(package_path('.ai/skills/empty'));
    ...
    expect(is_link(package_path('.claude/skills/empty')))->toBeFalse();
});
```
